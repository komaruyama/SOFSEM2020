...
Its transcript folds (downward in figures throughout this paper) in a zigzag manner macroscopically.
...
Unlike the counter of Geary et al., when a zig encounters overflow, it does not abort but extends the current value by 1 bit.
...
The binary representation of the initial value, $b_{k-1} b_{k-2} \cdots b_1 b_0$ is encoded on the seed in the following format
\begin{equation} \label{eq:zagencoding}
64{-}65{-}66{-}\left( \prod^0_{i = k-1} \bigl(  w_{Hn} w_{Rb} w_{Fb_i} w_{Lbn} \bigr) \right) w_{Hn}
\end{equation}
where $w_{Hn}, w_{Rb}, w_{Fb_i}, w_{Lbn}$ are sequences of bead types exposed downward by modules H, R, F, L when they fold into bricks Hn, Rb, F$b_i$, Lbn, respectively; these bricks can be found in Figs.~\ref{fig:halfadders},\ref{fig:rightturns}, \ref{fig:formatters}, and \ref{fig:leftturns}.
...
%64 65 66
%67 76 77 78 79 88 89 90 91 96
%97 102 103 108 109 114 115 120 121 126 127 132
%
%1 10 11 12 13 22 23 24 25 30
%
%31 36 37 42 43 48 49 54 55 64 65 66
%67 76 77 78 79 88 89 90 91 96
%97 108 109 120
...
\ \\
 ...
 \end{minipage}
 \end{tabular}
 \caption{All the 6 bricks of Module H. In zags, H always folds into the bottom-right brick Hn. }
